...
Mat AddGaussianNoise(const Mat mSrc, double Mean = 0.0, double StdDev = 10.0)
{
	Mat res(mSrc.size(), mSrc.type());
	...
	for (int Rows = 0; Rows < mSrc.rows; Rows++)
	{
		for (int Cols = 0; Cols < mSrc.cols; Cols++)
		{
			...
			Vec3b &Des_Pixel = res.at<Vec3b>(Rows, Cols);
...
int main(int argc, char *argv[]) {
	...
	waitKey(0);
	destroyWindow("MODELING_GAUS_NOISE");
	//Гаусс Фильтр
	...
	waitKey(0);
	destroyWindow("MY_MEDIAN_FILTER");
